...
#     Shared experts use the legacy 1D API; individual experts use bank-aware 2D.
...
from moe_layout import derive_mixed_workload_params
...
from bingo_kernel_args import (
    ...
    SnaxBingoKernelMoeInitOutputGapsArgs,
...
class HostBingoKernelMoEPrepareRequestArgs(LibHostBingoKernelMoEPrepareRequestArgs):
...
def validate_shared_individual_independence(
    ...
    for shared_node in shared_compute_nodes:
        ...
        if individual_predecessors:
            names = ", ".join(node.node_name for node in individual_predecessors[:4])
            raise ValueError(
...
def write_moe_config_header(
    ...
    if scheduler_bench_case is not None:
        ...
        lines += [
            f'#define MOE_SCHED_BENCH_CASE_NAME "{scheduler_bench_case.name}"',
            f"#define MOE_SCHED_BENCH_INPUT_TOKENS {scheduler_bench_case.input_tokens}u",
            f"#define MOE_SCHED_BENCH_ACTIVE_EXPERTS {scheduler_bench_case.active_experts}u",
            f"#define MOE_SCHED_BENCH_EXPERTS {scheduler_bench_case.n_experts}u",
            "static const uint32_t moe_scheduler_bench_expert_counts["
            "MOE_SCHED_BENCH_EXPERTS] __attribute__((aligned(64))) = {",
            f"    {counts}",
            "};",
            "static const uint16_t moe_scheduler_bench_topk_indices["
            "MOE_SCHED_BENCH_INPUT_TOKENS * 2u] __attribute__((aligned(64))) = {",
            f"    {topk_indices}",
            "};",
            "",
        ]
...
def define_memory_handles(params):
    ...
    # Individual experts use dense rows; shared experts retain the old padded
    # L1.5 input object and config.
    mh["L3_Sym_Input_A"] = BingoMemSymbol("input_A")
    mh["L3_Sym_Shared_Input_A"] = BingoMemSymbol("shared_input_A")
    ...
    # Shared down output preserves the old padded L1.5 row span per expert.
    mh["L3_Alloc_Shared_Down_Output"] = BingoMemAlloc(
        "l3_shared_down_out",
        size=num_se * params["l15_mode1_output_span_bytes"],
        mem_level="L3",
    )
    ...
    for prefix, cid in [("C2_indiv", CLUSTER_INDIV_A), ("C3_indiv", CLUSTER_INDIV_B)]:
        ...
        mh[f"{prefix}_L1_A"] = addr_offset(layout, params["bank_delta_local_a"])
        mh[f"{prefix}_L1_B_gate"] = addr_offset(layout, params["bank_delta_local_b0"])
        mh[f"{prefix}_L1_B_up"] = addr_offset(layout, params["bank_delta_local_b1"])
        mh[f"{prefix}_L1_B_down"] = addr_offset(
            layout,
            params["bank_mode1_region_offset"] + params["bank_delta_local_b0"],
        )
        mh[f"{prefix}_L1_D"] = addr_offset(layout, params["bank_delta_local_d0"])
    ...
    # C0/C1 shared experts retain one contiguous legacy L1.5 region per cluster.
    for prefix, cid in [
        ("C0", CLUSTER_SHARED_0),
        ("C1", CLUSTER_SHARED_1),
    ]:
...
def define_scheduler_bench_memory_handles(params):
    chip = 0
    ...
    mh = {
        ...
        "CAM": BingoMemAlloc("sched_bench_cam", size=8, mem_level="L3"),
        "Request": BingoMemAlloc(
            "sched_bench_request",
            size=256,
            mem_level="L3",
            condition=MOE_SW_SCHED_ABI_COND,
        ),
        "Schedule": BingoMemAlloc(
            "sched_bench_schedule",
            size=MOE_SCHEDULE_BYTES,
            mem_level="L3",
            condition=MOE_SW_SCHED_ABI_COND,
        ),
        "Runtime": BingoMemAlloc(
            "sched_bench_runtime", size=MOE_RUNTIME_STATE_BYTES, mem_level="L3"
        ),
        "Token_Refs": BingoMemAlloc(
            "sched_bench_token_refs", size=token_refs_bytes, mem_level="L3"
        ),
        "C2_Stage": BingoMemAlloc(
            "sched_bench_c2_stage", size=runtime_bytes, mem_level="L3"
        ),
        "C3_Stage": BingoMemAlloc(
            "sched_bench_c3_stage", size=runtime_bytes, mem_level="L3"
        ),
    }
    for prefix, cluster_id in (
        ("C2", CLUSTER_INDIV_A),
        ("C3", CLUSTER_INDIV_B),
    ):
        mh[f"{prefix}_Dyn"] = BingoMemAlloc(
            f"sched_bench_{prefix.lower()}_dyn",
            size=runtime_bytes,
            mem_level="L1",
            chip_id=chip,
            cluster_id=cluster_id,
        )
        mh[f"{prefix}_Static"] = BingoMemAlloc(
            f"sched_bench_{prefix.lower()}_static",
            size=params["static_arg_slot_bytes"],
            mem_level="L1",
            chip_id=chip,
            cluster_id=cluster_id,
        ...
        mh[f"{prefix}_Token_Refs"] = BingoMemAlloc(
            f"sched_bench_{prefix.lower()}_token_refs",
            size=token_refs_bytes,
            mem_level="L1",
            chip_id=chip,
            cluster_id=cluster_id,
        ...
        mh[f"{prefix}_L1_A"] = BingoMemAlloc(
            f"sched_bench_{prefix.lower()}_l1_a",
            size=64,
            mem_level="L1",
            chip_id=chip,
            cluster_id=cluster_id,
        ...
        mh[f"{prefix}_L1_D"] = BingoMemAlloc(
            f"sched_bench_{prefix.lower()}_l1_d",
            size=64,
            mem_level="L1",
            chip_id=chip,
            cluster_id=cluster_id,
...
def create_scheduler_bench_dfg(params, mh, scheduler_bench_case):
    bingo_dfg = BingoDFG(
        num_chiplets=1,
    ...
    experts = params["num_indiv_experts"]
    token_refs_bytes = experts * params["max_tokens_per_expert"] * 2
    ...
    node_prepare = BingoNode(
        ...
        kernel_args=HostBingoKernelMoEPrepareRequestArgs(
            expert_token_counts_addr=mh["Counts"],
            cam_state_addr=mh["CAM"],
            request_out_addr=mh["Request"],
            schedule_out_addr=mh["Schedule"],
            expert_token_refs_addr=mh["Token_Refs"],
            n_experts=experts,
            topk_indices_l3=mh["TopK"],
            M_total=params["M_total"],
            top_k=2,
            runtime_state_addr=mh["Runtime"],
            c2_stage_base=addr_offset(mh["C2_Stage"], MOE_RUNTIME_HEADER_BYTES),
            c3_stage_base=addr_offset(mh["C3_Stage"], MOE_RUNTIME_HEADER_BYTES),
            ...
            initial_cache_eid_c2=scheduler_bench_case.initial_cache_eid_c2,
            initial_cache_eid_c3=scheduler_bench_case.initial_cache_eid_c3,
        ),
    )
    ...
    node_execute = BingoNode(
        ...
        kernel_args=HostBingoKernelMoEExecuteArgs(
            schedule_addr=mh["Schedule"],
            runtime_state_addr=mh["Runtime"],
            expert_token_refs_addr=mh["Token_Refs"],
            c2_token_refs_l1_addr=mh["C2_Token_Refs"],
            c3_token_refs_l1_addr=mh["C3_Token_Refs"],
            token_refs_bytes=token_refs_bytes,
            cam_state_addr=mh["CAM"],
            input_A_l3_base=0,
            indiv_gate_B_l3=0,
            indiv_up_B_l3=0,
            ...
            c3_l1_d1_scratch=0,
            output_l3_addr=0,
            c2_active_state_l1_addr=mh["C2_Dyn"],
            c3_active_state_l1_addr=mh["C3_Dyn"],
            ...
            s1_block_count=params["indiv_N2"],
            s3_block_count=params["indiv_down_N2"],
...
def create_dfg(
    ...
    # padded span can be written back with the original host-iDMA API.
    shared_output_gap_init = {}
    for name, cluster_id in [
        ...
        node = BingoNode(
            ...
            assigned_core_id=DMA_CORE_ID,
            kernel_name="__snax_bingo_kernel_moe_init_output_gaps",
            kernel_args=SnaxBingoKernelMoeInitOutputGapsArgs(
                output_base=addr_offset(
                    mh[f"{name}_L1_Layout"],
                    params["l15_delta_local_mode1_d0"],
                ),
                row_payload_bytes=params["l15_mode1_payload_bytes_per_row"],
                row_stride_bytes=params["l15_a_row_stride"],
                rows=params["M_total"],
            ),
        )
        bingo_dfg.bingo_add_node(node)
        shared_output_gap_init[name] = node
    ...
    # ---- iDMA PATH: Router B/A, shared gate/config/A, individual gate/up ----
    # Shared C0/C1 use the original contiguous L1.5 destinations and 1D APIs.
    ...
    node_c0_load_A = BingoNode(
        ...
        kernel_name="__snax_bingo_kernel_idma_1d_copy",
        kernel_args=SnaxBingoKernelIdma1dCopyArgs(
            src_addr=mh["L3_Sym_Shared_Input_A"],
            dst_addr=addr_offset(mh["C0_L1_Layout"], params["l15_delta_local_a"]),
            size=params["l15_a_span_bytes"],
        ),
    )
    ...
    bingo_dfg.bingo_add_edge(node_xdma_c0_w2r, node_c0_shared_full)
    bingo_dfg.bingo_add_edge(shared_output_gap_init["C0"], node_c0_shared_full)
    ...
    bingo_dfg.bingo_add_edge(shared_output_gap_init["C1"], node_c1_shared_full)
...
def main():
    ...
    if scheduler_bench_case is not None and workload_mode == WORKLOAD_MODE_SCHEDULER_PREPARE:
        ...
        params["max_tokens_per_expert"] = scheduler_bench_case.input_tokens
        params["dynamic_slot_count"] = scheduler_bench_case.n_experts
    config_header_name = f"{params['app_name']}_config.h"
    write_moe_config_header(
    ...
    if scheduler_bench_case is None or workload_mode == WORKLOAD_MODE_SCHEDULER_ROUTED:
        ...
        post_execute_code = [
            'printf_safe("[MOE_SCHED_BENCH] case=%s tokens=%u active=%u '
            'experts=%u\\r\\n", MOE_SCHED_BENCH_CASE_NAME, '
            "MOE_SCHED_BENCH_INPUT_TOKENS, MOE_SCHED_BENCH_ACTIVE_EXPERTS, "
            "MOE_SCHED_BENCH_EXPERTS);",
            "__host_bingo_moe_print_phase_timing();",
        ]
