...
//
// CVA6-side MMIO helper for the MoE scheduler smoke test.
...
static inline uint64_t moe_sched_pack_head(moe_sched_head_t h)
{
    ...
    // Matches packed rem_head_t for E_MAX=64:
    // {valid, rem_index[6:0], eid[5:0], ntok[8:0], input_order[6:0], best_conc[15:0]}
...
static inline int moe_sched_wait_done(uint64_t *status_out)
{
    for (uint32_t i = 0; i < MOE_SCHED_TIMEOUT_POLLS; i++) {
        ...
        if (status & MOE_SCHED_STATUS_DONE) {
            if (status_out != NULL) {
                *status_out = status;
            }
            return 0;
...
static inline void moe_sched_make_sorted_rem(const moe_request_t *req,
...
static inline uint16_t moe_sched_count_active(const moe_sched_rem_item_t *rem,
                                              uint16_t n_experts)
{
    uint16_t n = 0;
...
static inline uint16_t moe_sched_total_conc(const moe_sched_rem_item_t *rem,
                                            uint16_t n_experts)
{
    uint16_t total = 0;
    for (uint16_t i = 0; i < n_experts; i++) {
        if (rem[i].active) {
            total = (uint16_t)(total + moe_sched_best_conc(rem[i].ntokens));
        }
    }
    return total;
}
...
{
    uint16_t active_count = moe_sched_count_active(rem, req->n_experts);
    uint16_t total_conc = moe_sched_total_conc(rem, req->n_experts);
    uint8_t h = 0;
    ...
    for (uint16_t i = 0; i < req->n_experts && h < 4u; i++) {
        ...
        moe_sched_head_t head = {
            ...
            .best_conc = moe_sched_best_conc(rem[i].ntokens),
        };

        switch (h) {
        case 0: moe_sched_write64(MOE_SCHED_HEAD0, moe_sched_pack_head(head)); break;
        case 1: moe_sched_write64(MOE_SCHED_HEAD1, moe_sched_pack_head(head)); break;
        case 2: moe_sched_write64(MOE_SCHED_HEAD2, moe_sched_pack_head(head)); break;
        default: moe_sched_write64(MOE_SCHED_HEAD3, moe_sched_pack_head(head)); break;
        }
        h++;
    }
}
...
static inline int moe_sched_remove_one(moe_sched_rem_item_t *rem,
                                       ...
                                       uint16_t idx)
{
    if (idx >= n_experts || !rem[idx].active) {
        return -1;
    }
    rem[idx].active = 0u;
    return 0;
                                              ...
                                              uint32_t verbose)
{
    ...
    for (uint16_t i = 0; i < req->n_experts; i++) {
        if (req->experts[i].ntokens == 0u ||
            req->experts[i].expert_id >= MOE_MAX_EXPERTS) {
            return -2;
        }
    }

    moe_sched_make_sorted_rem(req, rem);
    ...
    while (moe_sched_count_active(rem, req->n_experts) != 0u) {
        uint64_t status;
        uint64_t remove;
        uint64_t meta;
        uint64_t plan_word0;
        ...
        moe_sched_write_round_context(req, rem);
        ...
        meta = moe_sched_read64(MOE_SCHED_PLAN_META);
        remove_count = (uint32_t)(remove & 0x3u);
        remove_idx0 = (uint32_t)((remove >> 2) & 0x7fu);
        remove_idx1 = (uint32_t)((remove >> (2 + MOE_SCHED_NR_W)) & 0x7fu);
        plan_count = (uint32_t)(meta & 0x3u);
        slot_valid = (uint32_t)((meta >> 8) & 0x3u);

        if (remove_count == 0u || remove_count > 2u ||
            plan_count == 0u || plan_count > 2u ||
            slot_valid == 0u) {
            printf_safe("[SCHED_DRV] bad metadata round=%u remove=0x%lx meta=0x%lx\r\n",
                        round, remove, meta);
            return -5;
        }
        ...
        plan_word0 = moe_sched_read64(MOE_SCHED_PLAN0);
        plan[out_n++] = moe_sched_entry_from_task(moe_sched_unpack_task(plan_word0));

        if (plan_count == 2u) {
            plan_word1 = moe_sched_read64(MOE_SCHED_PLAN1);
            plan[out_n++] = moe_sched_entry_from_task(moe_sched_unpack_task(plan_word1));
        }

        if (moe_sched_remove_one(rem, req->n_experts, (uint16_t)remove_idx0) != 0) {
            printf_safe("[SCHED_DRV] bad remove_idx0 round=%u idx=%u\r\n",
                        round, remove_idx0);
            return -7;
        }
        if (remove_count == 2u &&
            moe_sched_remove_one(rem, req->n_experts, (uint16_t)remove_idx1) != 0) {
        ...
        if (verbose) {
            printf_safe("[SCHED_DRV] round=%u status=0x%lx remove=0x%lx meta=0x%lx out_n=%u\r\n",
                        round, status, remove, meta, out_n);
        }
                                              ...
                                              uint32_t case_id)
{
    ...
    for (uint16_t i = 0; i < n; i++) {
        uint64_t got_sig =
            ((uint64_t)got[i].valid << 0) |
            ((uint64_t)got[i].desc.cluster << 1) |
            ((uint64_t)got[i].desc.expert_id << 2) |
            ((uint64_t)got[i].desc.token_start_rank << 18) |
            ((uint64_t)got[i].desc.ntokens << 34) |
            ((uint64_t)got[i].desc.shape_s1 << 50) |
            ((uint64_t)got[i].desc.shape_s3 << 52) |
            ((uint64_t)got[i].desc.skip_s1 << 54) |
            ((uint64_t)got[i].desc.skip_s3 << 55) |
            ((uint64_t)got[i].desc.has_s2pf << 56) |
            ((uint64_t)got[i].allow_s4pf << 57);
        uint64_t exp_sig =
            ((uint64_t)exp[i].valid << 0) |
            ((uint64_t)exp[i].desc.cluster << 1) |
            ((uint64_t)exp[i].desc.expert_id << 2) |
            ((uint64_t)exp[i].desc.token_start_rank << 18) |
            ((uint64_t)exp[i].desc.ntokens << 34) |
            ((uint64_t)exp[i].desc.shape_s1 << 50) |
            ((uint64_t)exp[i].desc.shape_s3 << 52) |
            ((uint64_t)exp[i].desc.skip_s1 << 54) |
            ((uint64_t)exp[i].desc.skip_s3 << 55) |
            ((uint64_t)exp[i].desc.has_s2pf << 56) |
            ((uint64_t)exp[i].allow_s4pf << 57);
