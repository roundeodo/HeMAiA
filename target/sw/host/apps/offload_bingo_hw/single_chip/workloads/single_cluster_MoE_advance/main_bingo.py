...
def define_memory_handles(params):
    ...
    # Individual Expert 中间态 (不再需要 L3 Scatter Pool — DMA 直接写入 L1)
    # L3_Alloc_Indiv_Scatter_Pool 已移除：scatter 现在通过 SoC iDMA 直接
    # 从 L3 global_input_A 按 Token 粒度搬运到 L1 TCDM，节省 L3 内存。
...
def create_dfg(params, mem_handles):
    ...
    shared_nodes = {
        ...
        "host_swish": {},  # Key: (se_id, m)
        "host_glu": {},  # Key: (se_id, m)
    ...
    for se_id in range(params["shared_expert_number_k"]):
        for m in range(params["shared_swish_glu_M2"]):
            ...
            # 为保证 CVA6 多核/流水时不互相覆盖，每个 M 块赋予不同的 Temp Float 缓冲区偏移
            temp_buf_addr = addr_with_optional_offset(
                mem_handles["L3_Temp_Swish_Float_Buf"],
                ((se_id * params["shared_swish_glu_M2"]) + m)
                * valid_elements_shared
                * 4,
            )

            node_shared_swish = BingoNode(
                assigned_chiplet_id=0,
                assigned_cluster_id=0,
                assigned_core_id=host_core_id,
                kernel_name="__host_bingo_kernel_compute_swish_activation",
                kernel_args=HostBingoKernelComputeSwishActivationArgs(
                    gate_project_hw_data_addr=gate_out_row_addr,
                    swish_intermediate_buf_addr=temp_buf_addr,
                    valid_elements=valid_elements_shared,
                    swish_glu_scale_in_raw=scale_in_raw,
                ),
            )
            shared_nodes["host_swish"][(se_id, m)] = node_shared_swish
            bingo_dfg.bingo_add_node(node_shared_swish)

            # 节点：Host GLU
            node_shared_glu = BingoNode(
                assigned_chiplet_id=0,
                assigned_cluster_id=0,
                assigned_core_id=host_core_id,
                kernel_name="__host_bingo_kernel_compute_glu_multiplication",
                kernel_args=HostBingoKernelComputeGluMultiplicationArgs(
                    swish_intermediate_buf_addr=temp_buf_addr,
            ...
            shared_nodes["host_glu"][(se_id, m)] = node_shared_glu
            bingo_dfg.bingo_add_node(node_shared_glu)
    ...
    # =========================================================================
    indiv_nodes = {
        "scatter_dma": {},  # Key: (expert_id, m) — DMA scatter 直接写 L1
        ...
        "host_swish": {},  # Key: (expert_id, m)
        "host_glu": {},  # Key: (expert_id, m)
    ...
    for expert_id in range(params["expert_number_each_layer"]):
        ...
        # 每个 M tile 的最大 Token 数 (硬件 GEMM 一次处理的行数)
        max_tokens_per_tile = (
            params["individual_swish_glu_M1"]
            * params["meshRow"]
        )
        ...
        for m in range(params["individual_swish_glu_M2"]):
            ...
            # 节点：Host DMA Scatter — 直接按 Token 粒度从 L3 搬到 L1 TCDM
            # 不再需要中间 L3 scatter pool，也不做零填充
            node_scatter_dma = BingoNode(
                assigned_chiplet_id=0,
                assigned_cluster_id=0,
                assigned_core_id=host_core_id,
                ...
                kernel_args=HostBingoKernelScatterAndPadArgs(
                    ...
                    padded_scatter_pool_addr=current_l1_A,  # 直接写入 L1 TCDM
                    expert_token_counts_addr=mem_handles["L3_Alloc_Expert_Histogram"],
                    expert_memory_offsets_addr=mem_handles["L3_Alloc_Expert_Offsets"],
                    ...
                    max_padded_tokens=max_tokens_per_tile,
                    individual_expert_number_k=params["individual_expert_number_k"],
                ),
            )
            indiv_nodes["scatter_dma"][(expert_id, m)] = node_scatter_dma
            bingo_dfg.bingo_add_node(node_scatter_dma)
            ...
            temp_buf_addr = addr_with_optional_offset(
                mem_handles["L3_Temp_Indiv_Swish_Float_Buf"],
                ((expert_id * params["individual_swish_glu_M2"]) + m)
                * valid_elements_indiv
                * 4,
            )

            # 节点：Host Swish
            node_indiv_swish = BingoNode(
                assigned_chiplet_id=0,
                assigned_cluster_id=0,
                assigned_core_id=host_core_id,
                kernel_name="__host_bingo_kernel_compute_swish_activation",
            ...
            indiv_nodes["host_swish"][(expert_id, m)] = node_indiv_swish
            bingo_dfg.bingo_add_node(node_indiv_swish)

            # 节点：Host GLU
            node_indiv_glu = BingoNode(
                assigned_chiplet_id=0,
                assigned_cluster_id=0,
                assigned_core_id=host_core_id,
                kernel_name="__host_bingo_kernel_compute_glu_multiplication",
            ...
            indiv_nodes["host_glu"][(expert_id, m)] = node_indiv_glu
            bingo_dfg.bingo_add_node(node_indiv_glu)
    ...
    for se_id in range(params["shared_expert_number_k"]):
        for m in range(M2):
            ...
            # -----------------------------------------------------------------
            # store_gate_D → host_swish 这条直接边已被移除。
            # 原因：Gate 阶段完成后通过 store_gate_D→load_up_B→up_gemm→store_up_D 路径
            # 传递到 store_up_D，因此 store_gate_D 必先于 store_up_D 完成。
            # store_gate_D→host_swish 是冗余边，若保留则使 host_swish 有两个 Core2 前驱
            # (store_gate_D 和 store_up_D)，触发 dummy_check 插入导致 token aliasing 死锁。
            bingo_dfg.bingo_add_edge(
                shared_nodes["store_up_D"][(se_id, m, N2 - 1)],
                shared_nodes["host_swish"][idx_m],
            )

            bingo_dfg.bingo_add_edge(
                shared_nodes["host_swish"][idx_m], shared_nodes["host_glu"][idx_m]
            )
            bingo_dfg.bingo_add_edge(
                shared_nodes["host_glu"][idx_m], shared_nodes["load_act_A"][idx_m]
            )
    ...
    for expert_id in range(E):
        ...
        # ---------------------------------------------------------------------
        # 散布完成后，开启该专家的第一个 Load A
        bingo_dfg.bingo_add_edge(
            indiv_nodes["scatter_pad"][(expert_id,)],
            indiv_nodes["load_A"][(expert_id, 0)],
        )

        if expert_id < E - 1:
            # [核心保序] 下一个专家的 Load A 必须等当前专家的最后一块 GEMM 彻底让出 L1_A
            bingo_dfg.bingo_add_edge(
                indiv_nodes["down_gemm"][(expert_id, M2 - 1, N2_down - 1)],
                indiv_nodes["load_A"][(expert_id + 1, 0)],
            )
            # [dep_matrix 防护] 下一个专家的 Scatter 必须等当前专家的 host_glu 完成。
            # 原因：硬件调度器使用 dep_matrix[consumer_core][producer_core] 单比特矩阵
            # 作为依赖令牌。scatter → load_A 是 Core2→Core1 边，会置位
            # dep_matrix[1][2]。若 scatter[E+1] 过早运行（先于当前专家的 section4 完成），
            # 其 dep_set 令牌会被当前专家 pipeline 中的 Core1 任务错误消耗（令牌窃取），
            # 导致 load_act_A 在 host_glu 完成前就通过 dep_check。
            # 采用 host_glu → scatter[E+1] 同 Core2 FIFO 边来保证：
            #   (1) host_glu 的 dep_set 先被 load_act_A 正确消耗；
            #   (2) scatter[E+1] 随后才产生自己的 dep_matrix[1][2] 令牌。
            # 此边同时兼顾串行执行 Scatter 防止 CVA6 CPU 资源冲突的需求。
            bingo_dfg.bingo_add_edge(
                indiv_nodes["host_glu"][(expert_id, M2 - 1)],
        ...
        for m in range(M2):
            ...
            for n in range(N2_gate):
                ...
                if n == 0:
                    bingo_dfg.bingo_add_edge(
                        indiv_nodes["load_A"][idx_m], indiv_nodes["gate_gemm"][idx_mn]
                    )
                    # Gate B first tile should not run ahead of its corresponding A tile readiness.
                    bingo_dfg.bingo_add_edge(
                        indiv_nodes["load_A"][idx_m],
            ...
            for n in range(N2_gate):
                ...
                # load_A→up_gemm[n=0] 这条直接边已移除。
                # up_gemm[n=0] 通过传递路径 load_A→gate_gemm→load_up_B[n=0]→up_gemm[n=0]
                # 已隐式保序，直接边冗余。保留该边会使 load_A 扇出到 gate_gemm/load_gate_B/up_gemm
                # 三个跨核目标，minicompiler 需插入多层 dummy 节点消耗 dep_matrix token slot，
                # 在 NODE_TIMING 开启时 slot 耗尽导致 task76/77 死锁。
                bingo_dfg.bingo_add_edge(
            ...
            bingo_dfg.bingo_add_edge(
                ...
                indiv_nodes["host_swish"][idx_m],
            )

            bingo_dfg.bingo_add_edge(
                indiv_nodes["host_swish"][idx_m], indiv_nodes["host_glu"][idx_m]
            )
            bingo_dfg.bingo_add_edge(
                indiv_nodes["host_glu"][idx_m], indiv_nodes["load_act_A"][idx_m]
            )
            ...
            # -----------------------------------------------------------------
            if m < M2 - 1:
                # 保护 L1_A 缓冲: 下一个 m 的 Load A 等待当前 m 的所有 Down GEMM 彻底跑完
                bingo_dfg.bingo_add_edge(
                    indiv_nodes["down_gemm"][(expert_id, m, N2_down - 1)],
                    indiv_nodes["load_A"][(expert_id, m + 1)],
                )

            if m < M2 - 2:
                # L3 拥塞限制: 下下个 m 的 Load A 等待当前 m 的数据全部落盘 L3
                bingo_dfg.bingo_add_edge(
                    indiv_nodes["store_down_D"][(expert_id, m, N2_down - 1)],
                    indiv_nodes["load_A"][(expert_id, m + 2)],
                )
